...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        with dt[1]:
            ...
            ret = model(im, augment=augment, visualize=visualize)
            pred = ret[0]

            show_CAM("./CAM/CAM_result",
                     cv2.imread(path),
                     ret[1],
                     21,  # 指的是你想查看的类别 这个代码中我们看的是bear 所有在coco数据集中是21
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='./CAM/model_files/yolov5s.pt',
                        help='model path or triton URL')
    parser.add_argument('--source', type=str, default='./CAM/test_images/img.png',
